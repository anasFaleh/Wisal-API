...
export class RoundBeneficiaryController {
  constructor(
    private readonly roundBeneficiariesService: RoundBeneficiaryService,
  ) {}

  @Post()
  @Roles(Emp.ADMIN, Emp.DISTRIBUTER)
  @ApiOperation({
    summary: 'Allocate beneficiaries to a round (Admin, Distributer)',
  })
  ...
  @ApiOperation({
    summary: 'Get all allocations for a round (Admin, Distributer, Deliverer)',
  })
  @ApiParam({ name: 'roundId', description: 'Round ID (UUID)' })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary: 'Deliver a coupon to a beneficiary (Admin, Deliverer)',
  })
  ...
  @Roles(Emp.ADMIN, Emp.DELIVERER)
  @ApiOperation({
    summary: 'Search for a delivered coupon by its code (Admin, Deliverer)',
  })
